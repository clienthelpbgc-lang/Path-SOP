...
function TaskTemplateRowsSkeleton({ columnCount }: { columnCount: number }) {
  return (
    <>
...
type TaskTemplateTableProps = {
  columns: ColumnDef<TaskTemplate, unknown>[];
  data: TaskTemplate[];
  isLoading: boolean;
};

// Filtering, sorting and pagination all happen server-side (see
// TaskTemplateList / useTaskTemplates), so this only needs the core row
  ...
  return (
    <div className="rounded-xl border border-border bg-card">
      <Table>
        ...
        <TableBody>
          {isLoading ? (
            <TaskTemplateRowsSkeleton columnCount={columns.length} />
          ) : (

...
import { useCreateTaskTemplate } from "@/features/task/hooks";
...
type DuplicateTaskTemplatePresetDialogProps = {
  preset: TaskTemplatePreset | null;
  onOpenChange: (open: boolean) => void;
};

// Copies a platform-wide preset into a new, private, company-owned template
// that the company can then edit freely -- reuses createTaskTemplate
// unchanged, the same way CreateTaskTemplateDialog copies fields off an
// existing task.
export function DuplicateTaskTemplatePresetDialog({
  preset,
  onOpenChange,
}: DuplicateTaskTemplatePresetDialogProps) {
    ...
    mutate(
      {
        ...
        repeatUnit: preset.isRepeating
          ? (preset.repeatUnit ?? undefined)
          : undefined,
        repeatInterval: preset.isRepeating
          ? (preset.repeatInterval ?? undefined)
          : undefined,
        repeatDaysOfWeek:
          preset.isRepeating && preset.repeatUnit === "week"
            ? (preset.repeatDaysOfWeek ?? undefined)
            : undefined,
        isActive: true,
      },
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="duplicateTemplateName">Template name</Label>
            <Input
